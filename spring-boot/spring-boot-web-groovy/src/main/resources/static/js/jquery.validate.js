            ...
            if (validator.settings.submitHandler) {
              if (validator.submitButton) {
                ...
                hidden = $("<input type='hidden'/>").attr("name",
                    ...
                    validator.submitButton.value).appendTo(
          ...
          case "add":
            ...
            if (argument.messages) {
              settings.messages[element.name] = $.extend(
                  settings.messages[element.name], argument.messages);
            }
            break;
          case "remove":
            if (!argument) {
  ...
  $.extend($.validator, {
    ...
    defaults: {
      ...
      highlight: function (element, errorClass, validClass) {
        if (element.type === 'radio') {
          this.findByName(element.name).addClass(errorClass).removeClass(
              validClass);
        } else {
      ...
      unhighlight: function (element, errorClass, validClass) {
        if (element.type === 'radio') {
          this.findByName(element.name).removeClass(errorClass).addClass(
              validClass);
        } else {
    ...
    prototype: {
      ...
      check: function (element) {
        ...
        for (var method in rules) {
          ...
          try {
            ...
            result = $.validator.methods[method].call(this, val, element,
                rule.parameters);
            ...
            if (this.settings.debug && window.console) {
              console.log(
                  "exception occured when checking element " + element.id
      ...
      customMetaMessage: function (element, method) {
        ...
        var meta = this.settings.meta ? $(
                element).metadata()[this.settings.meta] : $(element).metadata();
      ...
      formatAndAdd: function (element, rule) {
        ...
        if (typeof message === "function") {
          ...
          message = $.validator.format(message.replace(theregex, '{$1}'),
              rule.parameters);
        }
        this.errorList.push({
          message: message,
      ...
      findByName: function (name) {
        ...
        return $(document.getElementsByName(name)).map(
            function (index, element) {
              return element.form === form && element.name === name && element
                  || null;
            });
      },
    ...
    methods: {
      ...
      range: function (value, element, param) {
        return this.optional(element) || ( value >= param[0] && value
            <= param[1] );
      },
